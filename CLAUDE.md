...
**ここはクライアントの PC**。間違えるとクライアントの大事なデータが消える。
以下のルールは **絶対に省略してはいけない**。

### 必ず事前確認する操作

以下の操作は、**Claude が勝手に実行してはいけない**。
必ずユーザー（IZAYOI）に明示的に確認し、「はい・実行して」と言われてから動く。

- ファイル・フォルダの **削除**（`rm`, `rm -rf`, `Remove-Item`, ファイル削除 API 全般）
- フォルダの **上書き・移動**（`mv`, `mkdir -p` で既存を潰す系）
- ファイルの **一括置換**（`sed -i`, `find -exec`, mass rename）
- Git の **破壊的操作**（`reset --hard`, `clean -f`, `checkout -- .`, `branch -D`, `push --force`）
- **設定ファイル**の書き換え（`.zshrc`, `.bashrc`, `.gitconfig`, ssh config, 環境変数等）
- **シンボリックリンクの削除**（リンク先の原本が消える誤解を生む）
- **外部 API への送信**（メール送信・Slack 投稿・カレンダー登録の確定等）
- **支払い・契約系の確定操作**（クレジットカード入力・サブスク申し込み）
- **ライセンス変更・公開設定変更**（リポジトリ Public 化等）

### 危険操作を提案するときの表示形式

ユーザーに確認を求める時は、**必ず以下の警告ブロック形式** で示す。さらっと文中で済ませない。
...
実行してよいですか？ (はい / いいえ / 詳細を見せて)
```

### 削除の場合は特に丁寧に

削除を提案する時は、上記の警告ブロックに加えて:

- **対象を一覧で見せる** （件数だけでなくファイル名を列挙）
- **ゴミ箱経由を優先** （macOS なら `trash`, Windows なら `Remove-Item -Recurse`）
- **バックアップ提案**: 「念のため `.local/backups/YYYY-MM-DD/` にコピーしてから消しますか？」を必ず先に聞く
- **「元に戻せない」を明示**: `rm -rf` は復元できない旨を毎回書く
...
>
> 操作内容: ファイルを削除します
> 対象: `.local/incoming/old_meetings/` 配下 12 ファイル
> 影響: **元に戻せません**（ゴミ箱経由ではなく完全削除）
> 代替案: `.local/backups/2026-06-01/` にコピーしてから削除する方が安全です
>
> 実行してよいですか？ (はい / いいえ / バックアップしてから / 詳細を見せて)

❌ 悪い例（こうしない）:

> 古いファイルを削除しますね。`rm -rf .local/incoming/old_meetings/` を実行します。

### 「危ない」と直感したら必ず言う

ルールに明記されていない操作でも、Claude が「これは戻せない・影響が広い」と感じたら、
**先に黄色信号を出す**。例:

> ⚠️ この操作は元に戻すのが難しい可能性があります。
> 理由: <なぜ危ないと判断したか>
> 進める前に確認させてください。

### 復旧手段の案内

危険操作の前に、復旧手段がある場合は **必ず教える**:

| 操作 | 復旧手段 |
|---|---|
| Git commit のミス | `git reflog` で 90 日以内なら戻せる |
| ファイル削除（ゴミ箱経由） | Finder のゴミ箱から復元可能 |
| `rm -rf` | **復元不可**（バックアップしかない） |
| ライセンス変更・公開設定 | Public→Private は可能だが、fork/キャッシュは残る |

### 例外

- `.gitignore` 済みファイル（一時ファイル等）でも、**ユーザーが作ったメモ** の可能性があるので削除前確認は省略しない
- `node_modules/` `__pycache__/` 等の自動生成物は削除確認を **簡易化** してよい（「再生成可能なファイルです、削除しますか？」程度）
